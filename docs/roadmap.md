...
| 3 | Receive / view / shelf / unshelf + broken-items shelf [раздел принять товар…полка сломанных вещей] | ✅ + 🎯 | Receive/putaway/view live since Ph14-15. **W1 adds:** explicit "pull from shelf" flow + a QUARANTINE zone for damaged goods (auto-suggested when grade is a damaged one) |
| 4 | Grading/photo rework with accept flow + notification [переделать оценку, фото] | 🎯 | **W2** — worker grades → item enters "review" queue → listing manager approves/rejects with reason → worker notified |
| 5 | Per-item conversation [переписка по товару — кто что] | 🎯 | **W2** — comments thread on the item card (wh + admin), joins the existing history timeline |
| 6 | Condition presets only — standardized reports [пресеты кондиций] | 🎯 | **W2** — preset note chips per condition (no free text unless "Other…"); grades become comparable/searchable |
| 7 | Pick timer + stats + who-did-what [таймер на сборку] | 🎯 | **W1** timers (claim→delivering timestamps already recorded) + **W3** stats screen |
...
| 🎯 W1 | Warehouse ops I: live pick board + notifications + timers + handoff + quarantine + unshelf + scanner CTA | design page → approve |
| 🎯 W2 | Warehouse ops II: grading approval flow + condition preset notes + item chat | design page → approve |
| 🎯 W3 | Warehouse ops III: productivity stats + bin browser | after W1/W2 |
