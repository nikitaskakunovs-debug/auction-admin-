...
export const FIN_SETTING_DEFAULTS = {
  ...
  storage_free_days: 7,
  /** Служебная метка старта платного хранения (epoch ms). Вещи, оплаченные
   *  до неё, начинают «лежать» с этого момента, а не со дня оплаты: правил
   *  тогда ещё не было, и выставлять долг задним числом нельзя.
   *  0 = ещё не запускалось; первый прогон крона записывает текущий момент. */
  storage_start_ms: 0,
