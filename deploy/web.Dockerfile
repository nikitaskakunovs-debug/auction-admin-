...
# Meta PageView: включается только после того, как базовый тег в GTM
# переведён на событие meta_page_view и передаёт eventID.
ARG NEXT_PUBLIC_META_PAGEVIEW
# Google AdSense: издатель (ca-pub-…) для базового скрипта верификации.
...
ENV NEXT_PUBLIC_API_URL=$NEXT_PUBLIC_API_URL \
    ...
    NEXT_PUBLIC_META_PAGEVIEW=$NEXT_PUBLIC_META_PAGEVIEW \
